...
type Team = {
  ...
  createdAt: string;
  updatedAt: string;
  isInitial?: boolean; // Campo opcional para identificar el equipo inicial
  members: TeamMember[];
};
...
export const TeamsProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchTeams = useCallback(async (projectId: string) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const updatedData = data.map((team: Team) => ({
        ...team,
        isInitial: team.isInitial || false, // Asegúrate de manejar el campo isInitial
      }));
  ...
  const createTeam = useCallback(async (
    team: { name: string; description: string; projectId: string; isInitial?: boolean },
    members: string[]
  ) => {
    setLoading(true);
    setError(null);
