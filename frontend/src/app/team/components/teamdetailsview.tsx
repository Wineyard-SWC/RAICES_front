...
const TeamDetailsView = ({ teamId }: TeamDetailsViewProps) => {
  ...
  const { teamMood, loading: moodLoading } = useTeamMood(
    currentTeam?.members || [],
    projectId
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-md h-full">
              {/* Mismo estilo de contenedor, altura completa */}
              {dataLoaded.users ? (
                <TeamAvatarsDisplay
                  teamId={teamId}
                ...
                <div className="flex flex-col justify-center items-center h-full py-20">
                  <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#4a2b4a]"></div>
                  <p className="mt-4 text-lg text-[#4a2b4a]">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 h-full">
                ...
                <div className="w-10 h-10 rounded-full bg-[#4a2b4a]/10 flex items-center justify-center mr-3">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0H3v-6a6 6 0 0112 0v6zm0 0h6v-6a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"
                    />
                  </svg>
                </div>
              ...
              <div className="mb-7">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-lg font-bold text-[#4a2b4a]">
                    {teamMetrics?.velocity} SP/Sprint
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (teamMetrics?.velocity || 0) * 2)}%`,
                    }}
                  ></div>
                </div>
              </div>

              {/* Team Mood - 🔥 USANDO DATOS REALES */}
              <div className="mb-7">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-gray-600">Team Mood</span>
                  <div className="flex items-center gap-2">
                    {moodLoading ? (
                      <div className="w-4 h-4 border-2 border-[#4a2b4a] border-t-transparent rounded-full animate-spin"></div>
                    ) : teamMood ? (
                      <>
                        <span className="text-2xl">
                          {teamMood.moodInterpretation.emoji}
                        </span>
                        <span
                          className={`text-sm font-medium ${teamMood.moodInterpretation.color}`}
                        >
                ...
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    className="bg-[#4a2b4a] h-3 rounded-full transition-all duration-500 ease-in-out"
                    style={{ width: `${teamMood?.averageMood || 0}%` }}
                  ></div>
                </div>
                {teamMood && (
                  <div className="text-xs text-gray-500 mt-1">
                    Based on{" "}
                    {teamMood.memberCount} member
                    {teamMood.memberCount !== 1 ? "s" : ""} biometric data
                  </div>
                )}
              </div>
