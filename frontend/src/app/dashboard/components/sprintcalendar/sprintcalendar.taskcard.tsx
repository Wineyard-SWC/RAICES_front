...
const CalendarTaskCard = ({ title, type, time, borderColor, bgColor, textColor, assignee, points }: Props) => {
  ...
  const [userName, setUserName] = useState<string>(assignee || '');
  const API_URL = process.env.NEXT_PUBLIC_API_URL;
  
  useEffect(() => {
    // Only fetch if we have an assignee
    if (assignee) {
      // Check if we have the user in localStorage first
      const cachedUsers = localStorage.getItem('cached_users');
      const users: Record<string, User> = cachedUsers ? JSON.parse(cachedUsers) : {};
      
      if (users[assignee]) {
        setUserName(users[assignee].name);
      } else {
        // Fetch the user data from the API
        fetch(`${API_URL}/users/${assignee}`)
          .then(response => {
            if (!response.ok) {
          ...
          .then(userData => {
            ...
            setUserName(userData.name || assignee);
            
            // Cache the user data
            users[assignee] = userData;
            localStorage.setItem('cached_users', JSON.stringify(users));
          })
          .catch(error => {
            console.error('Error fetching user:', error);
            // If there's an error, just use the assignee ID
  ...
  >
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center">
        {assignee && <span className="text-xs text-gray-500 mr-1">{userName}</span>}
