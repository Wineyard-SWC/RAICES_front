...
export default function AvatarCreatorContent() {
  ...
  // Obtener el userId de localStorage de forma segura (solo en el cliente)
  useEffect(() => {
        ...
        {isLoading ? (
          ...
          >
            <motion.div
              ...
              animate={{
                rotate: 360,
                borderRadius: ["50% 50%", "25% 75%", "50% 50%"],
              }}
              transition={{
                duration: 2,
                ease: "easeInOut",
          ...
          >
            <WelcomeAnimation avatarUrl={avatarUrl} gender={selectedGender} />
          </motion.div>
        )}
      </AnimatePresence>
    );
  ...
  // Esto nunca debería ocurrir, pero TypeScript necesita un retorno por defecto
  return null;
}