...
function StatusBadge({ status }: { status: string }) {
  // Mapeamos los estados a clases de colores consistentes con la aplicación
  const statusClasses = {
  ...
  // Formateamos el status para mostrarlo (primera letra en mayúscula)
  const displayStatus = status.charAt(0).toUpperCase() + status.slice(1)
  
  // Determinar la clase a usar
  const statusClass = 
    status === "active" ? statusClasses.active :
    status === "planning" ? statusClasses.planning :
    status === "completed" ? statusClasses.completed :
